...
# are executed in order (e.g., 0-1.sh → 1-2.sh → ...).
#
# Bump AGENTBOX_MIGRATION_VERSION when adding a new migration script.

set -euo pipefail

AGENTBOX_MIGRATION_VERSION=0
...
migration_check() {
    ...
    while [ "$v" -lt "$target" ]; do
        local next=$((v + 1))
        local script="$AGENTBOX_HOME/migrations/${v}-${next}.sh"
        if [ -f "$script" ]; then
            echo "[agentbox] Running migration: ${v} → ${next}"
            bash "$script" || {
                echo "[agentbox] ERROR: Migration ${v}-${next} failed. Aborting." >&2
                return 1
            }
        fi
        v="$next"
